...
import { deleteCachedClient } from "./client.cache.service.js";
...
export const updateClientService = async (id, payload) => {
  try {
    const isClientUpdated = await Clients.findByIdAndUpdate(
      id,
      { $set: payload },
...
export const incrementClientLead = async (id) => {
  try {
    ...
    const incremented = await Clients.findByIdAndUpdate(
      id,
...
export const getAllClients = async ({
  ...
  try {
    ...
    return serviceResponse(true, "Clients fetched successfully", {
      data: clients,
      meta: {
        total,
        page,
